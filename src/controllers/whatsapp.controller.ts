...
export const processImageUpload = async (message: any) => {
  ...
  try {
    const senderId = await message
      .getContact()
      .then((contact: any) => contact.id._serialized);

    if (message.hasMedia && message.body.toLowerCase().startsWith("!upload")) {
      if (senderId !== OWNER_ID) {
        logger.warn(
          `${FLAG} - Unauthorized image upload attempt by user: ${message.from}`,
        );
        await message.reply(
          "❌ *You are not authorized to upload assets to this system.*",
        );
        return;
      }
      ...
      const { caption, postTime, timeMatch } =
        await formatCaptionAndTime(message);
      ...
      if (!isValidTime && timeMatch) {
        await message.reply(
          '⚠️ Warning: Post time format seems off. Please use \'HH:MM\' (e.g., "17:00"). Defaulting to "12:00".',
        );
      }
